(function () {
  ...
  const i18n = {
    en: {
      ...
      'order.whatsappBtn': 'Chat on WhatsApp',
      ...
      'enquire.submitInstagram': 'DM on Instagram instead',
      'enquire.instagramNote': "Your message will be copied — just paste it into the DM once Instagram opens.",
      'enquire.note': "We'll never ask you to pay anywhere except inside that chat.",
      'enquire.customToggle': "I'd like a custom design (special mould, theme or shape)",
      'enquire.customLabel': 'Describe the design you have in mind',
      'enquire.customPlaceholder': 'e.g. a peacock-shaped mould, a specific colour theme...',
      'enquire.customHint': 'You can share reference photos directly in the chat once we connect.',
    ...
    mr: {
      ...
      'order.whatsappBtn': 'व्हॉट्सअ‍ॅपवर चॅट करा',
      ...
      'enquire.submitInstagram': 'त्याऐवजी इंस्टाग्रामवर DM करा',
      'enquire.instagramNote': 'तुमचा मेसेज कॉपी होईल — इंस्टाग्राम उघडल्यावर तो DM मध्ये पेस्ट करा.',
      'enquire.note': 'त्या चॅटशिवाय आम्ही कुठेही पेमेंट मागणार नाही.',
      'enquire.customToggle': 'मला खास डिझाइन हवं आहे (खास साचा, थीम किंवा आकार)',
      'enquire.customLabel': 'तुम्हाला हवं असलेलं डिझाइन सांगा',
      'enquire.customPlaceholder': 'उदा. मोराच्या आकाराचा साचा, ठराविक रंगसंगती...',
      'enquire.customHint': 'आपण कनेक्ट झाल्यावर तुम्ही चॅटमध्ये थेट संदर्भासाठी फोटो पाठवू शकता.',
  ...
  function updateActionLinks() {
    ...
    // Floating Instagram button — only shown once a link is set
    const floatIg = document.getElementById('floatingInstagram');
    if (floatIg) {
      floatIg.hidden = !hasInsta;
      if (hasInsta) floatIg.href = instagramUrl;
    }

    // Enquiry form's secondary Instagram submit button
    const enqIgBtn = document.getElementById('enquireInstagramBtn');
    const enqIgNote = document.getElementById('enquireInstagramNote');
    if (enqIgBtn) enqIgBtn.hidden = !hasInsta;
    if (enqIgNote) enqIgNote.hidden = !hasInsta;
  }
  ...
  // ---------------- Enquiry form -> WhatsApp / Instagram ----------------
  const enquireForm = document.getElementById('enquireForm');
  const customToggle = document.getElementById('enqCustomToggle');
  const customWrap = document.getElementById('enqCustomWrap');

  if (customToggle && customWrap) {
    customToggle.addEventListener('change', () => {
      customWrap.hidden = !customToggle.checked;
    });
  }
  ...
  function buildEnquiryMessage() {
    ...
    const customDesc = isCustom ? document.getElementById('enqCustomDesc').value.trim() : '';
    ...
    const lines = currentLang === 'mr'
      ? [
          ...
          isCustom ? 'हे खास डिझाइनसाठीचं विनंती आहे.' : '',
          customDesc ? `डिझाइन: ${customDesc}` : '',
          msg ? `टीप: ${msg}` : '',
        ]
      : [
          'Hi Modak Ghar!',
          ...
          date ? `Needed by: ${date}` : '',
          isCustom ? 'This is a custom design request.' : '',
          customDesc ? `Design idea: ${customDesc}` : '',
          msg ? `Note: ${msg}` : '',
        ];
  ...
  if (enquireForm) {
    enquireForm.addEventListener('submit', async (e) => {
      e.preventDefault();
      const channel = (e.submitter && e.submitter.getAttribute('data-channel')) || 'whatsapp';
      ...
      if (channel === 'instagram' && instagramUrl) {
        try {
          ...
          /* clipboard may be unavailable — Instagram still opens */
        }
